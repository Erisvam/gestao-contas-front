...
export class CartaoService {
  ...
  listarCartao(): Observable<CartaoResponse> {
    return this.httpClient.get<CartaoResponse>(this.rotaApi.getRotaCartoes(), {headers: this.rotaApi.getToken()})
  }

  consultarCartao(codigoCartao: string): Observable<ConsultaCartao>{
    return this.httpClient.get<ConsultaCartao>(this.rotaApi.getRotaCartoes(codigoCartao), {headers: this.rotaApi.getToken()});
  }

  deletarCartao(codigoCartao: string): Observable<any> {
    return this.httpClient.delete<any>(this.rotaApi.getRotaCartoes(codigoCartao), {headers: this.rotaApi.getToken()});
  }

  cadastrarCartao(bodyCartao: CadastroCartaoRequest): Observable<any>{
    return this.httpClient.post<any>(this.rotaApi.getRotaCartoes(), bodyCartao, {headers: this.rotaApi.getToken()});
  }
}
